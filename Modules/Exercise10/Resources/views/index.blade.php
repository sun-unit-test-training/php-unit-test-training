...
<div class="card">
    <div class="card-body">
        <h2 class="card-title display-4">{{ __('Kiểm tra giảm giá') }}</h2>
        <form method="post" action="{{ action('\Modules\Exercise10\Http\Controllers\Exercise10Controller@prepaid') }}">
            @csrf
            <!-- Card Level -->
            <div class="form-group">
                <label for="input-type">{{ __('Thứ hạng') }} {{ old('type') }}</label>
            ...
            <!-- Total price -->
            <div class="form-group">
                <label for="input-price">{{ __('Số tiền thanh toán') }}</label>
                <div class="input-group @error('price') is-invalid @enderror">
                    <input name="price" value="{{ old('price') }}" type="number" class="form-control @error('price') is-invalid @enderror" placeholder="{{ __('Số tiền thanh toán') }}">
            ...
            <div class="form-group">
                ...
                <div class="form-check form-check-inline">
                    ...
                    <label class="form-check-label" for="ballot__y">{{ __('Y') }}</label>
                </div>
                <div class="form-check form-check-inline">
                    <input name="ballot" class="form-check-input" type="radio" value="0" id="ballot__n" @if (old('ballot') == 0) checked @endif >
                    <label class="form-check-label" for="ballot__n">{{ __('N') }}</label>
                </div>
            </div>
            @if (isset($results))
            <div class="form-group col-md-6 offset-md-6">
                <dl class="row">
                    ...
                    <dt class="col-sm-8">{{ __('Số tiền thanh toán') }}</dt>
                    <dd class="col-sm-4 text-right">{{ $results['price'] }} {{ __('円') }}</dd>

                    <dt class="col-sm-8">{{ __('Số tiền được bonus') }}</dt>
                    <dd class="col-sm-4 text-right">{{ $results['bonus'] }} {{ __('円') }}</dd>

                    <dt class="col-sm-8">{{ __('Số tiền phải thanh toán') }}</dt>
                    <dd class="col-sm-4 text-right text-danger"><b>{{ $results['amount'] }} {{ __('円') }}</b></dd>
                </dl>
            </div>
            @endif
            <button type="submit" class="btn btn-primary">{{ ('Tính giá') }}</button>
        </form>
    </div>
</div>
@endsection
